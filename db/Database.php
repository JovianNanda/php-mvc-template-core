...
class Database
{
    ...
    public function stopMigrations()
    {
        ...
        $toDropMigrations = array_intersect($files, $appliedMigrations);
        ...
        if (!empty($newMigrations)) {
            $this->dropMigrations($newMigrations);
        } else {
    ...
    protected function dropMigrations(array $migrations)
    {
        $str = implode(",", array_map(fn($m) => "('$m')", $migrations));
        $statement = $this->pdo->prepare("DELETE FROM migrations WHERE migration = $str");
        $statement->execute();
    }
